...
function codeToThemedTokens(internal, code, options = {}) {
    const { lang = 'text', theme: themeName = internal.getLoadedThemes()[0], includeExplanation = true, } = options;
    if (isPlaintext(lang)) {
 ...
 *
 */
function flattenToken(merged, variantsOrder, cssVariablePrefix, defaultColor) {
    const token = {
...
export { FontStyle, addClassToHast, codeToHast, codeToHtml, codeToThemedTokens, codeToTokensWithThemes, createSingletonShorthands, createdBundledHighlighter, getHighlighterCore, getShikiContext, getShikiInternal, isPlaintext, isSpecialLang, loadWasm, toArray, toShikiTheme, tokenizeAnsiWithTheme };
